...
// ------------------------------------------------------------------
// IMPORTED IMAGE SEQUENCE DETECTION
// ------------------------------------------------------------------
function isImportedImageSequence(item) {
    if (!(item instanceof FootageItem) || !item.file) {
        return false;
    }

    // Only image formats can qualify as image sequences
    if (!hasExtension(
        item.file,
        "jpg,jpeg,png,tif,tiff,bmp,gif,exr,dpx,cin"
    )) {
        return false;
    }

    // Imported image sequences are not considered stills
    return item.mainSource && !item.mainSource.isStill;
}
...
// CREATE ROOT FOLDERS
// ------------------------------------------------------------------
var organizedFolder = app.project.items.addFolder("_Organized");
var assetsFolder = app.project.items.addFolder("01. Assets");
assetsFolder.parentFolder = organizedFolder;
var compsFolder = app.project.items.addFolder("02. Comps");
compsFolder.parentFolder = organizedFolder;

// ------------------------------------------------------------------
// ASSETS SUBFOLDERS (UPDATED NUMBERING)
// ------------------------------------------------------------------
var solidsFolder = app.project.items.addFolder("00. Solids");
solidsFolder.parentFolder = assetsFolder;
var imagesFolder = app.project.items.addFolder("01. Images");
imagesFolder.parentFolder = assetsFolder;
var graphicsFolder = app.project.items.addFolder("02. Graphics");
graphicsFolder.parentFolder = assetsFolder;
var seqFolder = app.project.items.addFolder("03. Seq");
seqFolder.parentFolder = assetsFolder;
var videosFolder = app.project.items.addFolder("04. Videos");
videosFolder.parentFolder = assetsFolder;
var audioFolder = app.project.items.addFolder("05. Audio");
audioFolder.parentFolder = assetsFolder;
var otherFolder = app.project.items.addFolder("Other");
otherFolder.parentFolder = assetsFolder;

// ------------------------------------------------------------------
// COMPS SUBFOLDERS
// ------------------------------------------------------------------
var precompsFolder = app.project.items.addFolder("01. Precomps");
precompsFolder.parentFolder = compsFolder;
var renderCompsFolder = app.project.items.addFolder("02. Render Comps");
...
var sequences = detectImageSequences(imageItems);
var sequencedItems = {};
var aeSequences = {};
for (var s = 0; s < sequences.length; s++) {
for (var si = 0; si < sequences[s].length; si++) {
sequencedItems[sequences[s][si].name] = s;
}
}

// Mark imported image sequences
for (i = 0; i < projectItems.length; i++) {
    item = projectItems[i];

    if (isImportedImageSequence(item)) {
        aeSequences[item.name] = true;
    }
}

// Create folders for sequentially numbered groups (non-imported sequences)
var groupFolders = {};
for (s = 0; s < sequences.length; s++) {
var groupName = "group" + ("0" + (s + 1)).slice(-2);
groupFolders[s] = app.project.items.addFolder(groupName);
...
item.parentFolder = renderCompsFolder;
}
moved = true;
}
// ------------------------------------------------------------------
...
else if (sequencedItems[item.name] !== undefined) {
item.parentFolder = groupFolders[sequencedItems[item.name]];
moved = true;
}
// Graphics
else if (hasExtension(item.file, "psd")) {
item.parentFolder = graphicsFolder;
moved = true;
}
// Images
else if (hasExtension(item.file, "jpg,jpeg,png,tif,tiff,bmp,gif,exr")) {
item.parentFolder = imagesFolder;
moved = true;
}
// Videos
else if (hasExtension(item.file, "mp4,mov,avi,mxf")) {
item.parentFolder = videosFolder;
moved = true;
}
// Audio
else if (hasExtension(item.file, "wav,mp3,aif,aiff,m4a")) {
...
alert("Error organizing project:\n\n" + err.toString());
}
app.endUndoGroup();
};
